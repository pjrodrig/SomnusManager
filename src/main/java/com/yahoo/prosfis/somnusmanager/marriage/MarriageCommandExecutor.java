						...
						if (sender instanceof Player) {
							...
							if (list != null && list.size() > 0) {
								...
								if (pclass == 1 || pclass == 101) {
									...
									sender.sendMessage(ChatColor.RED + "You are not a priest or an acolyte.");
								}
							}
						} else
							sender.sendMessage("Only players may issue this command.");
	...
	private void help(CommandSender sender) {
		...
		if (!(sender instanceof Player) || ((Player) sender).hasPermission("SomnusManager.admin")) {
			sender.sendMessage(new String[] { ChatColor.GOLD + "Arena marriage commands:",
					...
					ChatColor.AQUA + "/church set p1: " + ChatColor.GRAY + "set first player's position",
					ChatColor.AQUA + "/church set p2: " + ChatColor.GRAY + "set second player's position",
					ChatColor.AQUA + "/church set priest: " + ChatColor.GRAY + "set priest's position" });
		}
	}
}
